    ...
    public void fileRead(String filename) {
        ...
        try {
            while ((Line = br.readLine()) != null) { // reads every line of the file until it reaches the end
                ...
                event currEvent = new event( index, eventName, currMinutes);
...
//        int totalPossibleDays = totalTalksTime/perDayMinTime
    ...
    // prints the events
    public void eventsList(List<event> eventNames){
        ...
        for(int index=0;index<eventNames.size();index++)
        {
            ...
            if(!currEventDay.equals(eventNames.get(index).getDays()))
            {
                System.out.println(eventNames.get(index).getDays() + ":");
                System.out.println("");
                currEventDay = eventNames.get(index).getDays();
            }

            // prints the event name and timings
            System.out.println(eventNames.get(index).getName());

            // prints the lunch time
            if(eventNames.get(index).isLunchTime())
            {
                System.out.println(eventNames.get(index).getLunch());
            }

            // prints the networking time
            if(eventNames.get(index).isNetworkingTIme())
            {
                System.out.println(eventNames.get(index).getNetworking());
